...
All Jaxonomy numbers come from `benchmarks/public.py` — median of 3
runs on macOS-arm64 / JAX 0.9.2 / CPU. Reproduce with:
...
3. **Articulated quadruped throughput (T-019b)** — single-env MJX
...
| problem | metric | Jaxonomy CPU | T4 | A100 | H100 |
| --- | --- | ---: | ---: | ---: | ---: |
| cartpole_throughput (t_end=10s) | wall-s / sim-s | 0.0125 | _pending_ | _pending_ | _pending_ |
| quadruped_throughput (N=1000, t_end=5s) | env-s / wall-s | 13,822 | _pending_ | _pending_ | _pending_ |
| articulated_quadruped_throughput (N=1, nq=19/nv=18, t_end=5s) | wall-s / sim-s | 0.241 | _pending_ | _pending_ | _pending_ |
| sysid_convergence | iters / wall / param-MSE | 12 / 5.1 s / 5.1e-06 | _pending_ | _pending_ | _pending_ |
| linearization_vs_analytic | warm s / err vs analytic | 0.027 / 0.0 | _pending_ | _pending_ | _pending_ |

GPU columns are populated by re-running `benchmarks/public.py` on a real
NVIDIA runner (T-019a runner contract below). Empty cells mean we have
not yet collected the number on that device — they are *not* zeros and
not interpolated from CPU.

Cartpole simulates 10 simulated-seconds in ~125 ms on a single CPU
core (~80 simulated-s per wall-s). The simplified quadruped batch at
N=1000 runs 5,000 env-seconds in ~0.36 s wall (~1.4M env-steps/s at
a 100 Hz step).  The articulated quadruped (12-DoF body + floor
contact, MJX continuous-time, no controller) simulates 5 s of multi-
body physics in ~1.2 s wall (env-s/wall-s ≈ 4.2). Linearization
matches the analytic Jacobians to machine precision (the cubic
vanishes at x=0).
...
our CPU number isn't comparable, see T-019a); MuJoCo Playground RL-
policy-step throughput, which includes a learned controller and
contact-rich gait that our zero-control passive drop deliberately
excludes; massively-parallel articulated bodies on GPU (our
articulated quadruped is single-env CPU, batched MJX is filed as
`T-019b-followup-batched`); MPC inner-loop solve at 1 kHz (out of
scope for all three).
...
Baseline produced on macOS-arm64 / 68 GB RAM / JAX 0.9.2 / CPU only.
CI re-runs weekly on `ubuntu-latest` and uploads the JSON;
cross-runner CPU variance is ~10-20 %. GPU numbers are populated
opportunistically when a NVIDIA runner is available (T-019a) — see
"GPU runner contract" below for the recipe.

## GPU runner contract (T-019a)
...
* **Persistent JIT cache.** Setting
  `JAXONOMY_PERSISTENT_JIT_CACHE=1` (see `docs/jit_cache.md`) makes
...
* **Default dtype.** Jaxonomy enforces `float64` by default per T-005.
  ...
  public baseline — that would compare apples to oranges. If you want a
  separate float32 column, file `T-019a-followup-fp32` and add a new
  device key (e.g. `gpu_a100_f32`).
* **Batch-size scaling.** `quadruped_throughput` is fixed at N=1000 and
  ...
  GPU — that breaks the cross-device comparison. Larger-N is filed as
  `T-019b-followup-batched`.
...
| --- | --- | --- | --- | --- | --- |
| `gpu_t4` | _TBD: GitHub `gpu-t4` self-hosted, Colab T4, or Lambda Labs | NVIDIA T4 (16 GB) | 4 vCPU | 16 GB | parity with MuJoCo Playground README's published cartpole-1000-env throughput |
| `gpu_a100` | _TBD: AWS p4d.24xlarge, Modal a100-40gb sandbox_ | NVIDIA A100 40 GB | 8 vCPU | 64 GB+ | mid-tier reference for a fair head-to-head with JaxSim's published numbers |
| `gpu_h100` | _TBD: AWS p5.48xlarge, Modal h100 sandbox, GCP a3-highgpu_ | NVIDIA H100 80 GB | 8 vCPU | 128 GB+ | upper bound; JAX 0.9+ is required for full H100 SM_90 codegen |

When a number lands, replace the `_TBD: ...` cell with the exact
runner identifier (e.g. `Modal a100-40gb-<run-date>`,
`GHA self-hosted [linux, gpu, t4] runner #3`) so future regressions
can be triaged against the same hardware.

### Honest status

**No GPU numbers have been measured yet.** The schema, CLI flag, and
runner contract are in place; the `gpu_t4` / `gpu_a100` / `gpu_h100`
columns in `public_baseline.json` are explicit `null`s. Pending:
T4 / A100 / H100 runs on GitHub Actions GPU self-hosted runners or a
Modal / Replicate notebook (recipe above).
...
* **Simplified quadruped is a stand-in.** Four independent damped
  ...
  `articulated_quadruped_throughput` case (T-019b) covers the multi-
  body story.
* **Articulated quadruped is single-env (N=1)**, no controller, just
  ...
  / RL benchmark.  Batched MJX ensemble is filed as
  `T-019b-followup-batched`.
...
* **GPU columns not yet measured.** Schema, CLI flag, and runner
  contract for GPU runs ship under T-019a (see the "GPU runner
  contract" section above). Currently `gpu_t4` / `gpu_a100` /
  `gpu_h100` are explicit `null`s in `public_baseline.json`; numbers
  land when a NVIDIA runner is provisioned.
...
* **T-019a** — GPU benchmark numbers (T4 / A100 / H100). Schema +
  CLI flag + runner contract shipped (see "GPU runner
  contract" above); the `gpu_t4` / `gpu_a100` / `gpu_h100` rows in
  `public_baseline.json` are placeholder `null`s pending a real
  GPU run.
* **T-019b** — Real articulated-quadruped vs JaxSim on a fixed URDF.
