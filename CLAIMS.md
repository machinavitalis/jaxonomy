...
| C-016 | `JaxonomyDiagramSlave` FMU export honors exported diagram input ports as real FMI inputs, primes outputs during `exitInitializationMode` (no 0.0 placeholders before the first `doStep`), applies `EXPOSE_INITIAL_STATES` FMI parameters at initialization, registers each variable under the FMI type its signal dtype implies (`Boolean` / `Integer` / `Real`, non-Real pinned to `discrete` variability per FMI 2.0 §2.2.7), and the default-on cached-kernel `REUSE_SIMULATOR` path matches a fresh `simulate` bit-identically. | `CHANGELOG.md` ([3.1.0] Added, FMU export; [Unreleased] Fixed, FMI variable types) | `test/library/test_fmu_slave.py`, `test/library/test_fmu_export_binary.py` | green |
...
| C-023 | Influence-graph path attribution reproduces the end-to-end sensitivity of the assembled model: on the DC-motor loop, `attribute("speed:xc", "load_torque:out:out_0")` returns -1/J and agrees with central differences taken through the whole diagram to ~9e-12 relative. | `docs/examples/influence_graph_model_slicing.py` (section 2), `CHANGELOG.md` ([3.2.0] Added, influence graph) | `test/analysis/test_influence_example.py::test_attribution_matches_finite_differences`, `test/analysis/test_influence.py::TestPathAttribution` | green |
| C-024 | A `tau`-scaled path product equals that path's transfer magnitude at ω = 1/`tau`: for a chain of algebraic gains and *k* integrators it matches `(∏ gains)/ω**k` exactly across four decades of `tau`. | `jaxonomy/analysis/influence.py` module docstring, `docs/examples/influence_graph_model_slicing.py` (section 3) | `test/analysis/test_influence.py::TestPathAttribution::test_path_product_is_the_gain_at_one_over_tau` | green |
| C-025 | A quantitative influence slice is strictly smaller than the boolean one: on the 19-block DC-motor loop the boolean slice returns all 19 blocks and the 1% slice returns 18, dropping exactly the negligible stiction term; with the driver saturated the 1% slice returns 13 of 19, and `probe=0.9` recovers the six upstream control blocks whose local derivative is zero. | `docs/examples/influence_graph_model_slicing.py` (sections 3, 5), `CHANGELOG.md` ([3.2.0] Added, influence graph) | `test/analysis/test_influence_example.py::test_quantitative_slice_drops_exactly_the_stiction_term`, `::test_probe_recovers_the_blocks_a_saturated_derivative_writes_off` | green |
| C-026 | Trajectory-resolved weights capture an edge that only conducts during part of a run: the saturated driver's edge profile is 0 at the first snapshots and non-zero once the loop leaves the rail, and `reduce="max"` keeps the whole control path in the 1% slice. | `docs/examples/influence_graph_model_slicing.py` (section 6), `CHANGELOG.md` ([3.2.0] Added, influence graph) | `test/analysis/test_influence_example.py::test_trajectory_profile_shows_the_driver_leaving_the_rail`, `test/analysis/test_influence.py::TestTrajectoryMode` | green |
| C-027 | Influence-graph traversal is sound where a running-product cutoff is not: on a chain whose partial product dips to ~1e-4 at a cancelling junction before being amplified back, the contributor carrying 100% of the end-to-end influence is retained by a 1% slice, and a node in a feedback loop scores 1.0 against itself rather than the loop gain to the depth bound. | `jaxonomy/analysis/influence.py` (`_reach` docstring) | `test/analysis/test_influence.py::TestTraversalSoundness` | green |
| C-028 | Model-exchange FMU import (`ModelicaFMUME`) integrates an FMI 2.0 / 3.0 ME FMU with jaxonomy's own solver: Dahlquist matches the analytic `exp(-t)` to 1.5e-7, VanDerPol holds its limit cycle, and BouncingBall's event indicator drives zero-crossing localization plus the FMU's event iteration so the ball never falls through the floor. Against OpenModelica 1.27.0's own solution for one plant on its output grid, model exchange reaches 5.4e-8 m where co-simulation at `dt=0.01` reaches 4.0e-2 m. | `CHANGELOG.md` ([3.3.0] Added, model-exchange import), `README.md` | `test/library/test_fmu_me_import.py` (CI job `fmu-interop`, both FMI versions) | green |
| C-029 | An exported FMU runs under a master with no Python interpreter of its own: `fmusim` (Modelica Association reference simulator) validates it, instantiates it, and steps it, once the wrapper is built by `scripts/build_pythonfmu_wrapper.sh`. With the wrapper pythonfmu ships, the same FMU fails at `dlopen` with `undefined symbol: _Py_NoneStruct` while every static validator still reports it clean. | `CHANGELOG.md` ([3.3.0] Added, wrapper diagnostics), `KNOWN_GAPS.md` (FMU support) | `test/library/test_fmu_fmusim_master.py` (CI job `fmu-interop`) | green |
