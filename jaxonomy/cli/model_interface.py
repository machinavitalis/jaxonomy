...
from jaxonomy.dashboard.serialization.time_mode import (
    time_mode_node,
    time_mode_port,
...
from ..acausal import AcausalSystem
...
def get_acausal_signal_types(
    ...
    # NOTE these two are quite the hack
    ...
    for block_id, phleaf_outport_map in outports_maps.items():
        ...
        # Acausal blocks are assigned time_mode ACAUSAL; the per-port time-mode
        # computation below is intentionally disabled.
...
class AppInterface:
    ...
    def check(self, write_signals_json=None):
        ...
        # NOTE: ensure no top level inports/outports
        ...
        try:
            ...
            # jaxonomy internal errors should be raised, not collected.
            raise exc
