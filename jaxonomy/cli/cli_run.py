...
@click.command(
    help="Run a serialized jaxonomy model from the command line.\n\n\n"
...
@click.option(
    "--logsdir",
    type=click.Path(file_okay=False, dir_okay=True),
    help="Directory for Jaxonomy log files and timing statistics",
)
@click.option(
    "--no-output",
    is_flag=True,
...
def run(modeldir, model_json, output_dir):
    ...
    # NOTE: workdir=workdir not passed
    model = jaxonomy.load_model(modeldir, model_json, npydir=npydir, logsdir=logsdir)
    ...
    with open(os.path.join(modeldir, model_json), encoding="utf-8") as f:
        ...
        simulator_options = jaxonomy.SimulatorOptions(
            ...
            # NOTE: the default here is `variable_step`, but should be a specific method name
