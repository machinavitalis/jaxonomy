...
import jaxonomy
import jaxonomy.logging as jaxonomy_logger
from jaxonomy import Simulator
from jaxonomy.cli.run_optimization import run_optimization
from jaxonomy.backend import numpy_api as npa, DEFAULT_BACKEND
from jaxonomy.framework import build_recorder
from jaxonomy.framework.error import JaxonomyError
import jaxonomy.dashboard.serialization.ui_types as ts
from jaxonomy.dashboard.serialization import json_array, model_json
from jaxonomy.dashboard.serialization.ui_types import OptimizationResultsJson
...
class JaxonomyApplicationError(Exception):
    def __init__(self, internal_msg=None, user_msg=None):
        """An error that is raised by the Jaxonomy application.
...
def run_batch_simulation(parameters: list[dict[str, Any]]):
    ...
    sim_context = jaxonomy.load_model_from_dir(
    ...
    # NOTE: for ensemble sims, we only need to record those signals that we plot. Other
    ...
    # NOTE: show ensemble sim logs DASH-1738
    # NOTE: detect when dynamic parameters are used outside of a cache callback
...
def run_simulation(parameters: dict[str, dict] = None) -> dict[str, jnp.ndarray]:
    # NOTE: ideally we don't have to save the model json as a file and can directly
...
def _get_jaxonomy_error(e: Exception) -> JaxonomyError:
    if not e:
        return None
    if isinstance(e, JaxonomyError):
        return e
    return _get_jaxonomy_error(e.__cause__)
...
def run_jaxonomy(
    ...
    **run_fn_kwargs,
    ...
    jaxonomy_logger.set_log_level(log_level)
    jaxonomy_logger.unset_stream_handler()
    logs_filepath = os.path.join(work_dir, "logs", "logs.txt")
    jaxonomy_logger.set_file_handler(logs_filepath, formatter=JsonFormatter())
    ...
    jaxonomy.set_backend(DEFAULT_BACKEND)
    ...
    with set_cwd(work_dir):
        ...
        try:
            match task_type:
                ...
                case TaskType.SIMULATION:
                    # NOTE the returned value of a simulation run is not used,
        ...
        except JaxonomyError as e:
            traceback.print_exc()
            # See JsonFormatter in run_jaxonomy
            ...
            msg = _format_error(e)
            jaxonomy_logger.error(msg, extra={"extras": extras})
            raise JaxonomyApplicationError(str(e), user_msg=msg) from e

        except BaseException as e:
            traceback.print_exc()
            msg = _format_error(e)
            jaxonomy_error = _get_jaxonomy_error(e)
            if jaxonomy_error:
                error_object = ts.ErrorLog.from_error(e).to_api(omit_none=True)
                extras = {"__error__": error_object}
                jaxonomy_logger.error(msg, extra={"extras": extras})
            else:
                jaxonomy_logger.error(msg)
            raise JaxonomyApplicationError(str(e), user_msg=msg) from e

        finally:
            t1 = time.perf_counter()
            jaxonomy_logger.info("Total time: %.3fs", t1 - t0)
...
def main():
    ...
    # NOTE: ensure safety when reading back from it
