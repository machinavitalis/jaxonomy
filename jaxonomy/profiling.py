...
class Profiler:
    ...
    _min_profiles = {}
    _max_profiles = {}
    _logger = logging.getLogger("jaxonomy_profiler")
    _disable_profiling = os.environ.get("JAXONOMY_DISABLE_PROFILING", "0") == "1"
    # JAX JIT profiling must be disabled by default because it breaks various
    # models and advanced scenarios (autodiff, etc.)
    _enable_jaxjit_profiling = os.environ.get("JAXONOMY_JAXJIT_PROFILING", "0") == "1"
    _jaxjit_or_abort = os.environ.get("JAXONOMY_JAXJIT_OR_ABORT", "0") == "1"
    ...
    def stop(cls, name, raised=False):
        ...
        # ``pop`` (not ``__getitem__`` + ``del``) so a missing start() doesn't
        # raise KeyError inside ``__exit__`` and mask the original exception.
        t0 = cls._profiles.pop(name, None)
        if t0 is None:
            return
        dt = t1 - t0
    ...
    def jaxjit_profiledfunc(cls, fn, name=None):
        ...
        if not cls._enable_jaxjit_profiling or not isinstance(
            ...
            if cls._enable_jaxjit_profiling:
                if cls._jaxjit_or_abort:
                    raise RuntimeError(
                        f"JAXONOMY_JAXJIT_OR_ABORT=1 but {name} isn't a jitted function"
                    )
                cls._logger.warning(
        ...
        cls._logger.debug(
            "If compilation fails, consider setting JAXONOMY_JAXJIT_PROFILING=0"
            + " (or unset it))"
        )
