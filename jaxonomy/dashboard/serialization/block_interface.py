...
from jaxonomy import Parameter, library, LeafSystem
from jaxonomy.dashboard.serialization import model_json
import jaxonomy.library.state_machine as sm_lib
...
# description of the block and its actual implementation in Jaxonomy. In
...
def IntegratorDiscreteBlock(discrete_interval, initial_states, **kwargs):
    # T-037-followup: prefer per-block `dt` over the model-wide discrete_interval.
...
def _smooth_guard_residual_src(guard_src: str) -> str | None:
    """Return a smooth residual expression for a *simple comparison* guard.

    A StateMachine guard like ``"x > 0.5"`` is compiled to a boolean predicate
    whose gradient is identically zero, which blocks the event-time (saltation)
    gradient (T-NEW-sm-smooth-guard).  When the guard is a single comparison
    ``LHS OP RHS`` with ``OP ∈ {<, >, <=, >=}``, its zero-crossing coincides
    with ``LHS - RHS == 0``, so ``LHS - RHS`` is a smooth residual the
    event-time formula ``dt_e/dp = -∇g/D`` can differentiate.  The overall sign
    of the residual is irrelevant (``∇g`` and ``D`` flip together), so the
    comparison direction need not be matched.  Returns ``None`` for anything
    that is not a single ``<``/``>``/``<=``/``>=`` comparison (chained
    comparisons, ``==``/``!=``, boolean combinations, membership, etc.) — those
    have no single smooth threshold and keep the legacy (no-gradient) behaviour.
    """
    try:
        tree = ast.parse(guard_src, mode="eval")
    except SyntaxError:
        return None
    body = tree.body
    ...
    if len(body.ops) != 1 or len(body.comparators) != 1:
        return None  # chained comparison (a < b < c)
    if not isinstance(body.ops[0], (ast.Lt, ast.Gt, ast.LtE, ast.GtE)):
        return None  # ==, !=, is, in, ...
    try:
        left = ast.unparse(body.left)
        right = ast.unparse(body.comparators[0])
    except Exception:
        return None
    return f"({left}) - ({right})"
...
def _create_partial_transitions(
    ...
    grad_guard = None
    residual_src = _smooth_guard_residual_src(data.guard)
    if residual_src is not None:
        # NOTE: this is executed in the sandbox so it's safe
        grad_guard = eval(f"lambda {inputs}, {outputs}: {residual_src}")
    guard_id = registry.register_guard(guard, grad_guard)
...
def _create_state_machine_data(
    ...
    # can be stored in a jaxonomy state (i.e. not string)
...
def _wrap_discrete(block_cls):
    def _wrapped(*args, block_spec=None, discrete_interval=None, **kwargs):
        kwargs = _filter_init_kwargs(block_cls, kwargs)
        # T-037-followup: prefer per-block `dt` from the JSON parameters when
        # present; fall back to the model-wide `discrete_interval`. This lets
        # blocks (UnitDelay, ZeroOrderHold, EdgeDetection, RateLimiter,
        # DiscreteClock, DerivativeDiscrete, ...) round-trip a per-block dt
        # rather than always sourcing it from `model.configuration.sample_time`.
        dt = kwargs.pop("dt", None)
        return block_cls(
            discrete_interval if dt is None else dt,
            **_convert_bools(kwargs),
        )
