...
from jaxonomy.framework.error import (
...
from jaxonomy.framework.parameter import resolve_parameters
from jaxonomy.lazy_loader import LazyLoader
from jaxonomy.library import ReferenceSubdiagram
from jaxonomy.simulation import ResultsMode, ResultsOptions, SimulatorOptions
...
"""
Layer between the Jaxonomy app JSON output and the sim engine.
Presently, this works for model.json+list[submodel-uuid-ver.json] content
from both simworkerpy (historically "jaxonomy"; now jaxonomy) and simworker(go)(cmlc).
...
3] visit all the elements of diagrams dict, and build a jaxonomy diagram object for each, composing them as we go.
...
def load_model(
    ...
    _sv_raw = model_json_obj.get("schema_version", 0)
    if isinstance(_sv_raw, str):
        # Legacy Collimator/dashboard models stored schema_version as a string
        # (e.g. "3", "4"). Treat these as the legacy format without version-gating.
        import warnings

        warnings.warn(
            f"Model has a legacy string schema_version ({_sv_raw!r}) — "
            "assuming legacy format. Some features may not load correctly.",
            UserWarning,
        )
        schema_version = 0
    else:
        schema_version = int(_sv_raw) if _sv_raw else 0

    if schema_version > CURRENT_SCHEMA_VERSION:
        raise ValueError(
            f"Model schema version {schema_version} is newer "
            f"than this version of Jaxonomy supports "
            f"(max: {CURRENT_SCHEMA_VERSION}). "
            f"Please upgrade Jaxonomy."
        )
    if schema_version < 1:
        import warnings

        warnings.warn(
            "Model has no schema_version field — assuming "
            "legacy format. Some features may not load correctly.",
            UserWarning,
        )

    # T-016: apply any registered forward migrations to bring older
    # models up to the current schema version.  When no migrations are
    # registered (the steady-state case) this is a no-op.
    if schema_version < CURRENT_SCHEMA_VERSION:
        from .migrations import migrate_to_current, registered_versions

        if registered_versions():
            model_json_obj = migrate_to_current(
                model_json_obj, current_version=CURRENT_SCHEMA_VERSION,
            )
    ...
    if model.configuration.numerical_backend == "auto":
        model.configuration.numerical_backend = npa.active_backend
    jaxonomy.set_backend(model.configuration.numerical_backend)
    ...
    if model.configuration.workspace:
        # if it's not an empty dict, it should have a ref to the init script.
        # Not ideal, but works: detect the init-script ref via the workspace dict.
        if model.configuration.workspace.init_scripts:
            ...
            for param_name, param in model_parameters_py.items():
                ...
                # np.array_equal is element-wise and returns a plain bool, so
                # it is safe for array-valued parameters (a bare ``!=`` raises
                # "truth value of an array is ambiguous" for multi-element
                # arrays) while preserving scalar/None behaviour.
                if not np.array_equal(init_script_param, param):
                    logger.warning(
    ...
    recorded_signals = {}
    # T-037c: preserve the round-trip-stable Model.name and Model.uuid on the
    # reloaded Diagram, so subsequent serialize -> load -> serialize cycles
    # produce identical JSON. Fall back to the legacy defaults when a model
    # carries neither field (e.g. older saved formats).
    root_name = model.name or root_id
    root_ui_id = model.uuid or model.diagram.uuid
...
def eval_parameter(value: str, _globals: dict, _locals: dict):
    ...
    # NOTE: These are passed correctly when called from `eval_parameters`, but
...
def eval_parameters(
    ...
    # NOTE: WC-112 this should not be allowed
...
# NOTE: simplify this function. it's too long.
...
def make_subdiagram(
    ...
    # NOTE: correctly handle RefSubmodelConfiguration features
    ...
    for block_spec in diagram.nodes:
        ...
        try:
            # NOTE: refactor below contents of try into a function

            if block_overrides and block_name in block_overrides:
                # NOTE this was probably broken inside subdiagrams and looks extremely hacky anyway
            ...
            # branch for normal 'blocks'. not subdiagrams.
                ...
                if "finalize_script" in block_spec.parameters:
                    import warnings
                    warnings.warn(
                        f"Block '{block_name}' has a finalize_script "
                        f"parameter. finalize_script execution is not "
                        f"currently implemented and will be ignored.",
                        UserWarning,
                    )
                parameters = eval_parameters(
                    instance_parameters=block_spec.parameters,
                ...
                if block_spec.type == "core.StateMachine":
                    ...
                    # NOTE: the above condition is terrible
            ...
            # NOTE: Here we assume that the port order is the same in the frontend and jaxonomy
                    ...
                    if port.record or record_mode == "all":
                        if is_phealf_block:
                            ...
                            if i < 0 or i >= len(block.output_ports):
                                ...
                                # an invalid JSON with more i/o ports than what jaxonomy
            ...
            traceback.print_exc()
            if isinstance(exc, JaxonomyError):
    ...
    if goto_blocks:
        for link in diagram.links:
            ...
            if link.dst.node in goto_blocks.keys():
                ...
                # take note that this link will not create a jaxonomy diagram connection
        ...
        # branch for identifying acausal component blocks
            ...
            if block_spec.inputs:
                for idx, ioport in enumerate(block_spec.inputs):
                    # NOTE, we need a more explicit way to identify explicit ports of acausal blocks
...
def build_acausal_system(
    ...
    for link_uuid, link in links.items():
        ...
        # NOTE: this is a garnly nasty hack on top of the one below.
        ...
        # NOTE: this is a wicked nasty hack
    ...
    if acausal_system.outsym_to_portid is not None:
        for sym, acausal_system_port_id in acausal_system.outsym_to_portid.items():
            ...
            # NOTE: this is very hacky. should have the Sym object retain the
    ...
    # NOTE: saving in the object is pretty hacky but way simpler than passing around
...
def simulation_settings(
    ...
    # NOTE: Rename to max_checkpoints
