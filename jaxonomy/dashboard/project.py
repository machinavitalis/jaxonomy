...
from jaxonomy.dashboard.serialization import (
    model_json,
    from_model_json,
    to_model_json,
    SimulationContext,
)
from jaxonomy.dashboard.schemas import (
    FileSummary,
    ModelKind,
    ModelSummary,
    ProjectSummary,
)
from jaxonomy.dashboard import utils
...
class Project:
    """
    Represents a project in the Jaxonomy dashboard.
    """
    ...
    def upload_file(self, name: str, file: str, overwrite=True):
        ...
        Raises:
            api.JaxonomyApiError: If the file upload fails.
        """
        ...
        with open(file, "rb") as fp:
            ...
            except api.JaxonomyApiError as e:
    ...
    def upload_directory(
        ...
        This function will ignore files specified in the .gitignore or .jaxonomyignore file in the directory.
...
def get_project_by_name(project_name: str, **kwargs) -> Project:
    ...
    Raises:
        JaxonomyNotFoundError: If the project with the specified name is not found.
        JaxonomyApiError: If multiple projects with the same name are found.
    ...
    if len(results) == 0:
        raise api.JaxonomyNotFoundError(f"Project '{project_name}' not found")
    elif len(results) > 1:
        uuids = []
        for p in results:
            if p["is_default"]:
...
def get_or_create_project(name: str, **kwargs) -> Project:
    ...
    except api.JaxonomyNotFoundError:
...
class SimulationFailedError(api.JaxonomyApiError):
    pass
