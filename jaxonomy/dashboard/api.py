...
__API_BASE_URL__ = os.environ.get(
    "JAXONOMY_API_URL", "https://app.jaxonomy.com/api/v0"
)
__API_TOKEN__ = os.environ.get("JAXONOMY_API_TOKEN", None)

__HEADERS__ = {
    "X-Jaxonomy-API-Caller": "jaxonomy",
    "X-Request-ID": str(uuid4()),
    "Accept": "application/json",
    "X-Jaxonomy-API-Token": __API_TOKEN__,
}


class JaxonomyApiError(Exception): ...


class JaxonomyRetryableApiError(JaxonomyApiError): ...


class JaxonomyAuthenticationError(JaxonomyApiError): ...


class JaxonomyNotFoundError(JaxonomyApiError): ...
...
def set_api_token(token: str):
    ...
    __API_TOKEN__ = token
    __HEADERS__["X-Jaxonomy-API-Token"] = __API_TOKEN__
...
def call(
    ...
    if "X-Jaxonomy-API-Token" not in __HEADERS__:
        logger.warning(
            "No API token provided. Please set JAXONOMY_API_TOKEN envionment variable."
        )
    ...
    except JaxonomyRetryableApiError as e:
        if retries > 0:
