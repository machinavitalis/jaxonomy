...
from typing import TYPE_CHECKING, List, Hashable, Mapping, Optional
...
def _coerce_param_for_jit_cache(orig_val, new_val):
    """Promote a Python scalar / 0-d numpy array to ``jnp.asarray`` matching the
    original parameter's dtype.

    Without this, ``ctx.with_parameter("k", 0.4)`` stores ``0.4`` as a Python
    float leaf in the context pytree; ``jax.jit`` then treats it as a concrete
    static argument and retraces from scratch on every distinct value, making
    naive parameter sweeps quadratic in N
    (T-008-followup-with-parameter-trace-cache).
    """
    if npa.active_backend != "jax":
        return new_val
    if isinstance(new_val, (bool, int, float, complex, np.generic, np.ndarray)):
        dtype = getattr(orig_val, "dtype", None)
        if dtype is not None:
            return jnp.asarray(new_val, dtype=dtype)
...
def _reject_port_key(key) -> None:
    """Raise a helpful error if a context is indexed with a port.

    ``context[some_output_port]`` is a common mistake — a context is keyed by
    *system id*, not by port. Indexing with an ``OutputPort`` / ``InputPort``
    otherwise surfaces a confusing "System ID ... not found" assertion. Detect
    the port (by duck-typing, to avoid a circular import) and point the user at
    ``port.eval(context)`` instead. (T-B2-followup-context-port-key)
    """
    # An InputPort / OutputPort exposes ``.eval`` and ``.system``; a system id
    # (the valid key type) does not.
    if hasattr(key, "eval") and hasattr(key, "system") and hasattr(key, "direction"):
        name = getattr(key, "name", repr(key))
        direction = getattr(key, "direction", "")
        if direction == "out":
            raise TypeError(
                f"Context is keyed by system id, not by port. To read output "
                f"port {name!r}, call `port.eval(context)` (e.g. "
                f"`results.context` → `my_block.output_ports[i].eval(context)`), "
                f"not `context[port]`."
            )
        raise TypeError(
            f"Context is keyed by system id, not by port. You indexed it with "
            f"input port {name!r}; index by a system id (e.g. "
            f"`context[block.system_id]`) or read an output via "
            f"`block.output_ports[i].eval(context)`."
        )
