...
class ModelInitializationError(JaxonomyError):
...
class PythonScriptTimeNotSupportedError(PythonScriptError):
    ...
    # this is a breaking change from previous versions of jaxonomy, thus
...
class LegacyBlockConfigurationError(StaticError):
    """A block is not supported in the current version of Jaxonomy."""
    ...
    def default_message(self):
        return (
            ...
            "incompatible with the current version of the Jaxonomy simulation "
            "engine. Please re-create the block manually in UI or review the "
            "instantiation code."
        )


class StaticParameterError(JaxonomyError):
