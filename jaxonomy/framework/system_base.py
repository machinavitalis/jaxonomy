...
Systems are the basic building blocks of a model in the jaxonomy framework.
...
class _PortAccessList(list):
    """A ``list`` of ports that raises an actionable error on bad indexing.

    Indexing an empty / too-short ``block.output_ports`` (or ``input_ports``)
    otherwise surfaces a bare ``IndexError: list index out of range`` that
    doesn't name the block or the likely cause — a forgotten
    ``declare_continuous_state_output()`` / ``declare_output_port(...)``.
    (T-B1-followup-empty-output-ports)
    """

    __slots__ = ("_owner_name", "_direction")

    def __init__(self, owner_name: str, direction: str):
        super().__init__()
    ...
    def __getitem__(self, index):
        try:
            ...
            kind = "output" if self._direction == "out" else "input"
            if len(self) == 0:
                hint = ""
                if self._direction == "out":
                    hint = (
                        " The block declared no output ports — did you forget "
                        "`declare_continuous_state_output()` (for a block with "
                        "continuous state) or `declare_output_port(...)`?"
                    )
                else:
                    hint = (
                        " The block declared no input ports — did you forget "
                        "`declare_input_port(...)`?"
                    )
                raise IndexError(
                    f"{self._owner_name!r} has no {kind} ports (tried "
                    f"index {index!r}).{hint}"
                ) from None
            raise IndexError(
                f"{self._owner_name!r} has {len(self)} {kind} port(s); "
                f"index {index!r} is out of range."
            ) from None


def _input_port_eval_callback(context, owner, port_index):
    """Input port eval closure for :meth:`SystemBase.declare_input_port` (via ``partial``)."""
    return owner._eval_input_port(context, port_index)


def _build_priority_tiebreak(system: "SystemBase"):
    """Build a ``(rate_key, priority_key, name)`` tiebreak callable for
    discrete-update scheduling, or ``None`` when no leaf has an explicit
    ``priority`` attribute set.

    T-105-followup-priority-scheduler-hook: this hooks the read-only
    ``compute_execution_order`` ordering (shipped by
    T-105-fu-tasking-priority) into the actual scheduler that runs
    inside :meth:`SystemBase.handle_discrete_update`.

    Returning ``None`` when no priorities are declared is load-bearing —
    it keeps the default-off path byte-equivalent to the pre-followup
    scheduler (T-022a's ``str(system_id)`` Kahn tiebreaker, declaration
    order on the diagonal branch).

    The callable returned takes a ``system_id`` (the key used internally
    by :func:`topological_sort` and as a fallback for events whose
    leaf is not in the diagram, e.g. for a single-LeafSystem
    ``handle_discrete_update``) and returns the sort tuple.  System ids
    that don't map to a known leaf get a worst-case key so they sort
    after every known block, never before.
    """
    # A Diagram exposes ``leaf_systems``; a bare LeafSystem doesn't.
    # The latter has nothing to reorder (one block, no ambiguity), so
    # short-circuit.
    leaves = getattr(system, "leaf_systems", None) or []
    if not leaves:
        return None

    # Bail out unless at least one leaf has an explicit priority — this
    # is the byte-equivalence guard.  ``getattr`` + identity check on
    # ``None`` is exactly what ``infer_block_priority`` does internally;
    # we mirror that here to avoid an import cycle into
    # ``jaxonomy.simulation`` from a ``framework`` module.
    has_any = any(getattr(leaf, "priority", None) is not None for leaf in leaves)
    if not has_any:
        return None

    # Lazy import — ``rate_groups`` lives in ``jaxonomy.simulation`` and
    # we are inside ``jaxonomy.framework``.  Doing the import at module
    # load time would create a cycle.
    from ..simulation.rate_groups import (
    ...
    key_by_id: dict[Hashable, tuple] = {}
    for leaf in leaves:
        st = infer_block_sample_time(leaf)
        rate_key = (rate_order[st.kind], st.period or 0.0)
        p = infer_block_priority(leaf)
        prio_key = (0, p) if p is not None else (1, 0)
        key_by_id[leaf.system_id] = (rate_key, prio_key, leaf.name or "")

    # Worst-case fallback for unknown ids: sort after every known block.
    fallback = ((max(rate_order.values()) + 1, float("inf")), (2, 0), "~")

    def _key(sid):
        return key_by_id.get(sid, fallback)
