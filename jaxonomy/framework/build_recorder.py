...
if TYPE_CHECKING:
    ...
    from ..acausal.component_library.component_base import ComponentBase
...
def _add_acausal_domain_to_imports(full_mod_path: str):
    ...
    __IMPORTS__.add(f"from jaxonomy.acausal import {domain}")
...
def create_block(block: Union["LeafSystem", "ComponentBase"], init_fn, *args, **kwargs):
    ...
    if isinstance(block, LeafSystem):
        __IMPORTS__.add("from jaxonomy import library")
...
def create_acausal_compiler():
    __IMPORTS__.add("from jaxonomy.acausal import AcausalCompiler")
...
@_action
def create_eqn_env():
    __IMPORTS__.add("from jaxonomy.acausal import EqnEnv")
    __BUILD_CMDS__.append(CreateEqnEnvCommand())


@_action
def create_acausal_diagram():
    __IMPORTS__.add("from jaxonomy.acausal import AcausalDiagram")
...
def create_parameter(args):
    ...
    if args.get("name", False):
        __IMPORTS__.add("from jaxonomy.framework import Parameter")
