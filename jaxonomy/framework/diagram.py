...
from .leaf_system import _check_values_compatible, LeafSystem
...
def _diagram_rewrite_child_refs(
    diagram: "Diagram", old_child: SystemBase, new_child: SystemBase
) -> None:
    """Swap ``old_child`` for ``new_child`` in all port locator maps."""

    def swap(loc: Tuple[SystemBase, int]) -> Tuple[SystemBase, int]:
        sys_, idx = loc
        return (new_child, idx) if sys_ is old_child else loc

    diagram.connection_map = {
        swap(inp): swap(outp) for inp, outp in diagram.connection_map.items()
    }
    diagram._input_port_map = {
        swap(k): v for k, v in diagram._input_port_map.items()
    }
    diagram._output_port_map = {
        swap(k): v for k, v in diagram._output_port_map.items()
    }
    diagram._inv_output_port_map = {
        k: swap(v) for k, v in diagram._inv_output_port_map.items()
    }


def _diagram_refresh_exported_outputs_for_child(
    diagram: "Diagram", child: SystemBase
) -> None:
    """Re-bind diagram-level output callbacks to ports on ``child``."""
    for loc, d_port_idx in diagram._output_port_map.items():
        sys_, sub_idx = loc
        if sys_ is not child:
            continue
        src = child.output_ports[sub_idx]
        diagram.configure_output_port(
            d_port_idx,
            src.eval,
            prerequisites_of_calc=[src.ticket],
        )


def _diagram_rebuild_leaf_systems(d: "Diagram") -> None:
    """Rebuild ``leaf_systems`` like :meth:`Diagram.__post_init__`."""
    d.leaf_systems = []
    for sys in d.nodes:
        if isinstance(sys, Diagram):
            _diagram_rebuild_leaf_systems(sys)
            d.leaf_systems.extend(sys.leaf_systems or [])
            sys.leaf_systems = None
        else:
            d.leaf_systems.append(sys)
