...
import jaxonomy
from jaxonomy.dashboard.serialization.ui_types import NodePathType
from jaxonomy.framework.context import ContextBase
from jaxonomy.framework.diagram import Diagram
from jaxonomy.framework.error import CallbackIsNotDifferentiableError
from jaxonomy.framework.port import OutputPort
from jaxonomy.framework.system_base import SystemBase
from jaxonomy.library import PID, Adder, Integrator, PIDDiscrete, Power, SourceBlock
from jaxonomy.library.custom import CustomPythonBlock
from jaxonomy.library.utils import extract_columns, read_csv
...
def _process_pid_blocks(
    ...
    if pid_gains_init is None:
        # Open question: when no initial gains are supplied, should we seed from
        # the blocks' existing non-zero kp/ki/kd instead of a flat default?
...
class OptimizableModel(Optimizable):
    def __init__(
        ...
        if job_type == "design":
            ...
            pid_gains_min = 0.0
            pid_gains_max = 1e03  # NOTE: get from user? 1000 seems reasonable
...
class OptimizableModelWithStochasticVars(OptimizableWithStochasticVars):
    def __init__(
        ...
        if job_type == "design":
            ...
            pid_gains_min = 0.0
            pid_gains_max = 1e03  # NOTE: get from user
...
def jobs_router(
    ...
    # NOTE: Extend this feature for other jobs too
    ...
    if algorithm in UI_NAME_TO_METHOD_OPTAX:
        ...
        optim = NLopt(
            optimizable=opt_model,
            opt_method=opt_method,
            ftol_rel=1e-04,  # NOTE: defaults, in case we want to get these from UI
            ftol_abs=1e-06,
            xtol_rel=1e-04,
            xtol_abs=1e-06,
            cons_tol=1e-06,
        ...
        optim = IPOPT(
            optimizable=opt_model,
            options={"disp": 0},  # NOTE: More options from UI?
        )
    ...
    # optimize() may return an OptimizationResult (new API) whose ``.params``
    # holds the dict; the job-specific handling below expects that bare dict.
    if hasattr(optimal_parameters, "params"):
        optimal_parameters = optimal_parameters.params
