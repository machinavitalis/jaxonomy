...
from jaxonomy.framework.state import LeafState
from jaxonomy.framework.error import StaticError, BlockInitializationError
...
class StateMachineRegistry:
    ...
    def __init__(self):
        ...
        # Parallel list (same index as ``_guards``) of optional *smooth* guard
        # residuals used only for the event-time (saltation) gradient — the
        # boolean trigger guards above have zero gradient, so a smooth
        # ``lhs - rhs`` residual is supplied here when the guard is a simple
        # comparison.  ``None`` => no smooth surface (no event-time gradient).
        # T-NEW-sm-smooth-guard.
        self._grad_guards = [None, None]
    ...
    @property
    def grad_guards(self):
        return self._grad_guards

    @property
    def actions(self):
        return self._actions

    def register_guard(self, guard: Callable, grad_guard: Callable = None) -> int:
        if _is_lambda_body_true(guard):
            return ALWAYS_TRUE_GUARD_ID
        self._guards.append(guard)
        self._grad_guards.append(grad_guard)
...
class StateMachineData:
    ...
    def to_padded_arrays(self) -> "StateMachineData":
        ...
        for state_id, state in self.states.items():
            ...
            padded_transitions = [
                StateMachineTransition(
                    ...
                    dst=trans.dst,
                    action_ids=npa.array(trans.action_ids, dtype=npa.int32),
                )
        ...
        padded_initial_actions = npa.array(padded_initial_actions, dtype=npa.int32)
...
def _choose(idx, arr):
    return npa.choose(jnp.array([idx]), arr, mode="clip")
