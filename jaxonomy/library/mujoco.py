...
from jaxonomy.backend import io_callback, get_dispatcher
...
from jaxonomy.logging import logdata, logger
...
class MuJoCoBase(LeafSystem):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not file_name:
            raise JaxonomyError("XML file name must be specified", system=self)

        self.use_mjx = use_mjx
        if use_mjx and get_dispatcher().active_backend != "jax":
            raise JaxonomyError(
        ...
        if key_frame_0 is not None:
            ...
            if qpos_0 is None:
                ...
                self.qpos_0 = jnp.asarray(qpos_0)

            if qvel_0 is None:
                self.qvel_0 = jnp.zeros(self.nv)
            else:
                self.qvel_0 = jnp.asarray(qvel_0)

            if act_0 is None:
                self.act_0 = jnp.zeros(self.na)
            else:
                self.act_0 = jnp.asarray(act_0)
    ...
    def _add_custom_output(
        ...
        def fn_kwargs():
            ...
            if has_mocap_pos and self.enable_mocap_pos:
                # mocap_pos is stored as (num_mocap, 3) in MuJoCo but _data.mocap_pos
                # is shaped (1, n) for a single-body mocap; squeeze to (n,) to match
                # the expected input port layout.
