...
from jaxonomy.framework.error import StaticError, ErrorCollector
...
def _reshape(A, B, C, D):
    ...
    # Accept plain lists/scalars too — __init__ passes the raw constructor args
    # (the initialize() path already resolves them to arrays).
    A, B, C, D = npa.asarray(A), npa.asarray(B), npa.asarray(C), npa.asarray(D)
    if len(A.shape) == 1:
        n = 1
        A = A.reshape((n, n))
    else:
...
class LTISystemBase(LeafSystem, ABC):
    ...
    def _init_state(self, A, B, C, D, initialize_states=None):
        ...
        if initialize_states is None:
            initialize_states = npa.zeros(self.n)
        else:
            initialize_states = npa.array(initialize_states)
        ...
        if initialize_states.size == 1 and self.n > 1:
            initialize_states = npa.ones(self.n) * initialize_states.ravel()
...
#: Alias for :class:`PID` — continuous-time PID controller.
#: ``PIDContinuous`` is provided for symmetry with ``PIDDiscrete``.
PIDContinuous = PID
