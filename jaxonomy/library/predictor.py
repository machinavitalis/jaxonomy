...
# T-B5-followup-predictor-aliases: users naturally reach for
# ``PyTorchPredictor`` / ``TensorFlowPredictor`` (the blocks are *predictors*),
# but the classes are named ``PyTorch`` / ``TensorFlow``. Expose the
# ``*Predictor`` spellings as aliases so either name imports and constructs the
# same block.
PyTorchPredictor = PyTorch
TensorFlowPredictor = TensorFlow
