...
_load_error_msg = (
    ...
    "and HermiteSimpsonNMPC. Install 'jaxonomy[nmpc]' for NMPC blocks support."
)
...
class NonlinearMPCIpopt(NonlinearMPCBase):
    ...
    def __init__(
        ...
        # Warm start reuses the previous IPOPT solution as the next solve's
        # initial guess. The cache lives host-side (``solve`` runs under
        # ``jax.pure_callback``), so it persists across the major steps of a
        # simulation. ``None`` until the first successful solve.
    ...
    def solve(self, time, state, *inputs):
        ...
        # Warm start from the previous solution when enabled and available,
        # otherwise start from zeros.
        if self.warm_start and self._warm_start_cache is not None:
            optvars_guess = self._warm_start_cache
        else:
        ...
        # Cache the converged solution to seed the next solve when warm
        # starting.
        if self.warm_start:
            self._warm_start_cache = optvars_sol
