...
def _reduce(base_feature_names, feature_names, coefficients):
    ...
    zero_columns = npa.all(coefficients == 0, axis=0)
    ...
    if len(feature_names_filtered) == 0:
        ...
        coefficients_filtered = npa.zeros((nx, nxu))
...
def train(
    ...
    differentiation_objects_map = {
        ...
        "finite_difference": ps.differentiation.FiniteDifference(order=1, axis=0),
        "savitzky_golay": ps.differentiation.SmoothedFiniteDifference(
            smoother_kws={"window_length": 11, "polyorder": 3}, axis=0
        ),
        "spectral": ps.differentiation.SpectralDerivative(axis=0),
    }
    ...
    # Convert npa arrays to numpy arrays explicitly for Sindy fit
    ...
    time = np.array(time) if isinstance(time, npa.ndarray) else time
    ...
    return equations, base_feature_names, feature_names, npa.array(coefficients)
