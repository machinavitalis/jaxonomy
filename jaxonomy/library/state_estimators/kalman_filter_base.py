...
class KalmanFilterBase(LeafSystem, ABC):
    ...
    def _update(self, time, state, *inputs, **params):
        ...
        # Note: the correction below duplicates computation from _feedthrough_output_x_hat.
        # The two are kept separate because JAX does not provide a mechanism to share
        # intermediate computations across method calls at trace time — both paths are
        # JIT-compiled independently.  If this ever becomes a performance bottleneck,
        # caching via jaxonomy's DependencyTicket machinery could be explored.
