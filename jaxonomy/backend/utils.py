...
def make_array(
    ...
    if default_value is not None:
        default_value = npa.array(default_value, dtype=dtype)
    else:
        default_value = npa.zeros(shape, dtype=dtype)
    ...
    if not npa.issubdtype(default_value.dtype, npa.number) and not (
