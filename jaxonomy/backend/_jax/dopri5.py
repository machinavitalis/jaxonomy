...
class Dopri5Solver(ODESolverImpl):
    ...
    def _initialize_state(self, func, t0, xc0, _mass, unravel, *args, dt=None):
        ...
        # T-038a-followup-integrator-f32: pin ``t0`` to a strong JAX f64 dtype
        # so the ``while_loop`` carry stays type-stable when the state dtype
        # (and therefore ``dt``) is f32.  A weak-typed Python float ``t0``
        # would otherwise let ``t + dt`` produce strong f32 in the body
        # (weak f64 + strong f32 → f32), causing a carry mismatch with the
        # weak-f64 ``t``/``t_prev``/``t_return`` slots on the next iteration.
        # Promoting at the entry boundary keeps the f64 default path
        # numerically identical (still f64 throughout) — only the weak→strong
        # bit changes.
        t0 = jnp.asarray(t0, dtype=xc0.dtype)
    ...
    def optimal_step_size(
        ...
        return jnp.clip(next_step, min=self.hmin, max=self.hmax)

    def attempt_rk_step(self, func, boundary_time, solver_state):
        y, f, t, dt = solver_state.step_variables
        dt_dtype = dt.dtype
        # T-038a-followup-integrator-f32: keep ``dt`` in its declared dtype so
        # the ``while_loop`` carry stays type-stable.  ``boundary_time - t`` is
        # in the time dtype (typically f64) which would otherwise promote a
        # f32 ``dt`` to f64 here.
        dt = jnp.clip(
            dt, min=self.hmin, max=(boundary_time - t).astype(dt_dtype)
        )
        ...
        # Preserve dt's dtype across step-size control (defensive: matches the
        # carry slot dtype even if internal promotion lifts the intermediate).
        next_dt = self.optimal_step_size(dt, error_ratio).astype(dt_dtype)
        error_step_size_too_small(t, next_dt, boundary_time, self.enable_autodiff)
        # If the optimal next step size is at or below hmin the step size controller
        # can no longer reduce dt — the while_loop condition (~accepted) would never
        # become True, hanging forever.  Force-accept the current (inaccurate) step
