...
def get_paths(pytest_request, testdir_=None, test_name_=None):
    ...
    # Derive the repo root by walking up from this file. Fragile — it assumes
    # the installed layout jaxonomy/jaxonomy/testing/runtime_test.py.
    jaxonomy_root = pathlib.Path(__file__).parent.parent.parent
    test_root = pathlib.Path(os.path.join(jaxonomy_root, "test"))
...
def copy_to_workdir(test_paths, *globs, dirs_exist_ok=False):
    ...
    for g in globs:
        ...
        for src in glob.glob(g):
            ...
            # Copy directories recursively, files individually.
...
def calc_err_and_test_pass_conditions(
    ...
    if sol.dtype == "bool":
        ...
        rel_err_den = np.multiply(
            np.clip(np.abs(sol), a_min=rel_err_den_clip, a_max=np.inf),
            rel_err_den_sign,
        )
