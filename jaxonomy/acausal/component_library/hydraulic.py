...
class MassflowSource(HydraulicOnePort):
    ...
    def __init__(self, ev, name=None, M=1.0, enable_massflow_port=False):
        ...
        if enable_massflow_port:
            m_flow = self.declare_symbol(ev, "M", self.name, kind=SymKind.inp)
        else:
            m_flow = self.declare_symbol(ev, "M", self.name, kind=SymKind.param, val=M)

        # Keep sign convention aligned with fluid MassflowSource.
        self.add_eqs([sp.Eq(self.M.s, -m_flow.s)])
...
class PressureSensor(ComponentBase):
    ...
    def __init__(
        ...
        if enable_port_b:
            ...
            self.add_eqs([sp.Eq(M1.s, 0), sp.Eq(M2.s, 0)])
        else:
