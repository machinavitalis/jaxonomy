...
# ---------------------------------------------------------------------------
# T-104 phase 2 (acausal extension): canonical SI units per domain.
# ---------------------------------------------------------------------------
#
# Each acausal physical domain has a conventional (flow, potential)
# pair. The classic textbook table:
#
#   electrical:     flow=current (A),     potential=voltage (V)
#   rotational:     flow=torque (N·m),    potential=ang.velocity (rad/s ≡ 1/s)
#   translational:  flow=force (N),       potential=velocity (m/s)
#   thermal:        flow=heat-flow (W),   potential=temperature (K)
#   hydraulic:      flow=mass-flow (kg/s), potential=pressure (Pa)
#   fluid:          flow=mass-flow (kg/s), potential=pressure (Pa)
#
# These constants surface on the per-domain ``PortBase`` subclasses as
# ``flow_units`` / ``pot_units`` class attributes (T-104 phase 2). The
# unit values themselves live here so external tooling
# (provenance manifests, unit-summary printers, downstream UI) can
# also enumerate the canonical mapping.

ACAUSAL_UNIT_AMPERE   = Unit(dims=(0, 0, 0, 1, 0, 0, 0), name="A")
ACAUSAL_UNIT_VOLT     = Unit(dims=(1, 2, -3, -1, 0, 0, 0), name="V")
ACAUSAL_UNIT_NEWTON   = Unit(dims=(1, 1, -2, 0, 0, 0, 0), name="N")
ACAUSAL_UNIT_M_PER_S  = Unit(dims=(0, 1, -1, 0, 0, 0, 0), name="m/s")
ACAUSAL_UNIT_NM       = Unit(dims=(1, 2, -2, 0, 0, 0, 0), name="N*m",
                             physical_quantity="torque")
ACAUSAL_UNIT_RAD_PS   = Unit(dims=(0, 0, -1, 0, 0, 0, 0), name="rad/s",
                             physical_quantity="angular_velocity")
ACAUSAL_UNIT_KELVIN   = Unit(dims=(0, 0, 0, 0, 1, 0, 0), name="K")
ACAUSAL_UNIT_WATT     = Unit(dims=(1, 2, -3, 0, 0, 0, 0), name="W")
ACAUSAL_UNIT_PASCAL   = Unit(dims=(1, -1, -2, 0, 0, 0, 0), name="Pa")
ACAUSAL_UNIT_KG_PER_S = Unit(dims=(1, 0, -1, 0, 0, 0, 0), name="kg/s")
...
class PortBase:
    ...
    Attributes:
        ...
        flow_units (Unit | None):
            The canonical SI unit of the flow variable for this
            port's domain (T-104 phase 2 extension to the acausal layer).
            Overridden by each domain-specific PortBase subclass.
            ``None`` here on the base class so existing custom port
            subclasses outside the standard domains keep working.
        pot_units (Unit | None):
            The canonical SI unit of the potential variable for this
            port's domain. ``None`` on the base class.
    """
...
class ElecPort(PortBase):
    ...
    flow_units = ACAUSAL_UNIT_AMPERE
    pot_units = ACAUSAL_UNIT_VOLT
...
class FluidPort(PortBase):
    ...
    fluid = None
    flow_units = ACAUSAL_UNIT_KG_PER_S
    pot_units = ACAUSAL_UNIT_PASCAL
...
class RotationalPort(PortBase):
    ...
    flow_units = ACAUSAL_UNIT_NM
    pot_units = ACAUSAL_UNIT_RAD_PS
...
class ThermalPort(PortBase):
    ...
    flow_units = ACAUSAL_UNIT_WATT
    pot_units = ACAUSAL_UNIT_KELVIN
...
class TranslationalPort(PortBase):
    ...
    flow_units = ACAUSAL_UNIT_NEWTON
    pot_units = ACAUSAL_UNIT_M_PER_S
...
class ComponentBase(metaclass=AcausalBlockMeta):
    ...
    def __init__(self):
        self.ports: Dict = {}
        # T-002a: insertion-ordered dicts used as ordered sets. Sets iterate in
        # hash order, which is randomised per-process and causes non-
        # deterministic acausal compilation outputs (state-vector size and
    ...
    def declare_1D_lookup_table(
        ...
        xp_array = npa.array(xp)
        yp_array = npa.array(yp)
    ...
    def declare_2D_lookup_table(
        ...
        lut_f = sp.Function("npa.interp2d")(
            xp_sym.s,
            yp_sym.s,
            zp_sym.s,
            x_expr,
    ...
    def declare_conditional(
        ...
        if smooth and non_bool_zc_expr is not None:
            # Smooth approximation of a binary switch using tanh blending.
            alpha = 0.5 * (1.0 + sp.tanh(smooth_scale * non_bool_zc_expr))
            cond_f = alpha * then_expr + (1.0 - alpha) * else_expr
        else:
            cond_f = sp.Piecewise(
                (then_expr, if_expr),
                (else_expr, True),
