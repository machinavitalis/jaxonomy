...
class HeatflowSensor(ThermalTwoPort):
    ...
    def __init__(
        ...
        # T-031: a passive flow sensor has T1 = T2 (no temperature drop)
        # AND Q1 + Q2 = 0 (heat passes through; no storage). Without
        # the latter, two sensors in series produce equation/variable
        # count mismatch in Pantelides because each sensor leaves Q2
        # unconstrained on its own side, and chained sensors compound
        # the deficiency.
        self.add_eqs([
            sp.Eq(self.T1.s, self.T2.s),
...
class HeatflowSource(ComponentBase):
    ...
    Sign convention (Modelica): the through-variable ``Q`` at each port is
    positive when heat flows *into* this component. So with the default
    ``enable_port_b=True`` and ``Q_flow > 0``, heat is pumped *from* the
    element connected at ``port_a`` *into* the element connected at
    ``port_b`` (i.e. ``Q_a = +Q_flow``, ``Q_b = -Q_flow``). To deliver
    positive heat to a thermal mass, wire the mass at ``port_b`` (with
    ``Q_flow > 0``), or at ``port_a`` with ``Q_flow < 0``. With
    ``enable_port_b=False`` the source applies an absolute heatflow at
    ``port_a`` (``Q_flow > 0`` extracts heat from the connected mass).

    Args:
        Q_flow (number):
            Heatflow value when ``enable_heat_port=False``.
        enable_heat_port (bool):
            When true, the heatflow value is from an input signal. When
            false, heatflow value is from 'Q_flow'.
        enable_port_b (bool):
            When port_b enabled, applies heatflow between port_a and port_b.
            When port_b disabled, applies absolute heatflow to port_a.
    """
