...
class IdealGasAir(FluidBase):
    ...
    def __init__(
        ...
        # rather than defaulting to 0.0. A zero-density IC causes div-by-zero during
        # Jacobian evaluation at t=0. This is the intended initialization approach for
        # all fluid media classes.
        h_ic, u_ic, d_ic = self.get_h_u_d_ics(P_ic, T_ic)
        self.init = {"p": P_ic, "T": T_ic, "h": h_ic, "u": u_ic, "d": d_ic}

    def gen_eqs(self, ev, p, h, T, u, d, name_prefix=""):
        # name_prefix accepted for call-site compatibility with the fluid.py
        # volume components (ClosedVolume/Accumulator/...); this medium adds
        # no namespaced internal symbols so it is unused here.
        eqs = [
...
class WaterLiquidSimple(FluidBase):
    ...
    def __init__(
        ...
        # Pre-compute physical initial conditions from (P_ic, T_ic) — same rationale
        # as IdealGasAir: prevents div-by-zero from zero density during IC solving.
        h_ic, u_ic, d_ic = self.get_h_u_d_ics(P_ic, T_ic)
        self.init = {"p": P_ic, "T": T_ic, "h": h_ic, "u": 0.0, "d": d_ic}

    def gen_eqs(self, ev, p, h, T, u, d, name_prefix=""):
        # name_prefix accepted for call-site compatibility (see IdealGasAir);
        # unused — this reduced medium introduces no namespaced symbols.
        # Simplified incompressible-fluid approximation: density is constant, enthalpy is
        # proportional to temperature (h = cp*T), and internal energy equals enthalpy
        # (valid when pressure work p/d is negligible compared to thermal energy).
        # This is intentionally a reduced model — not full thermodynamic fidelity.
        eqs = [
...
class WaterLiquid(FluidBase):
    ...
    def gen_eqs(self, ev, p, h, T, u, d, return_gamma=False, name_prefix=""):
        pi = p / self.pstar
        tau = self.Tstar / T

        base_name = f"{name_prefix}waterIF97R1_inst"
        kind = SymKind.var
        gp = Sym(
            ev,
            sym_name="gp",
