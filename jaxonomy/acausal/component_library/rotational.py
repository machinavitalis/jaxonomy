...
class Friction(RotationalTwoPort):
    ...
    def __init__(self, ev, name=None, Tc=1.0, Wbrk=0.1, C=None, Tbrk=None):
        ...
        if C is not None:
            ...
            trq_total += C.s * Vrel.s

        if Tbrk is not None:
            Tbrk = self.declare_symbol(
                ev,
                "Tbrk",
                self.name,
                kind=SymKind.param,
                val=Tbrk,
                validator=lambda Tbrk: Tbrk >= Tc.val,
                invalid_msg=f"Component {self.__class__.__name__} {self.name} must have Tbrk>=Tc",
            )
            sqrt_2 = sp.sqrt(2.0)
            w_scaled = Vrel.s / (Wbrk.s * sqrt_2)
            stribeck_term = (
                sqrt_2
                * sp.sqrt(sp.E)
                * (Tbrk.s - Tc.s)
            ...
            trq_total += stribeck_term

        self.add_eqs([sp.Eq(self.t1.s, trq_total)])
...
class SpeedSource(ComponentBase):
    ...
    def __init__(
        ...
        if enable_speed_port:
            w_ref = self.declare_symbol(ev, "w_ref", self.name, kind=SymKind.inp)
        else:
            w_ref = self.declare_symbol(
                ev, "w_ref", self.name, kind=SymKind.param, val=w_ref
            )
        ...
        if enable_flange_b:
            ...
            self.add_eqs(
                [
                    # source does not store angular momentum
                    sp.Eq(0, t1.s + t2.s),
                    # prescribed relative speed
                    sp.Eq(w1.s - w2.s, w_ref.s),
                ]
            )
        else:
            # prescribed absolute speed
            self.add_eqs([sp.Eq(w1.s, w_ref.s)])
...
class GearRatio(Gear):
    """
    Convenience alias of Gear with no efficiency map.
    """

    def __init__(self, ev, name=None, ratio=1.0):
        super().__init__(ev, name=name, r=ratio)


class Clutch(RotationalTwoPort):
    """
    ...
    def __init__(
        ...
        k_clutch = self.declare_symbol(
            ...
            invalid_msg=f"Component {self.__class__.__name__} {self.name} must have k_clutch>=0",
        )
        engage = self.declare_conditional(
            ev,
            control.s >= 0.0,
            control.s,
            0.0,
        ...
        self.add_eqs(
            [
                sp.Eq(self.t1.s, k_clutch.s * engage.s * (self.w1.s - self.w2.s)),
                sp.Eq(0, self.t1.s + self.t2.s),
            ]
        )


class LeadScrew(ComponentBase):
    """
    Rotational-translational ideal lead screw.
    pitch [m/rev] => lead_per_rad = pitch / (2*pi).
    """

    def __init__(self, ev, name=None, pitch=0.01):
        self.name = self.__class__.__name__ if name is None else name
        super().__init__()
        t, ang, w, alpha = self.declare_rotational_port(ev, "shaft")
        f, x, v, a = self.declare_translational_port(ev, "flange")
        self.port_idx_to_name = {-1: "shaft", 1: "flange"}
        pitch = self.declare_symbol(
            ev,
            "pitch",
            self.name,
            kind=SymKind.param,
            val=pitch,
            validator=lambda x: x > 0.0,
            invalid_msg=f"Component {self.__class__.__name__} {self.name} must have pitch>0",
        )
        lead = self.declare_symbol(ev, "lead", self.name, kind=SymKind.var)
        self.add_eqs(
            [
                sp.Eq(lead.s, pitch.s / (2.0 * sp.pi)),
                sp.Eq(v.s, lead.s * w.s),
                sp.Eq(-f.s, t.s / lead.s),
            ]
        )
