...
class MassflowSource(FluidOnePort):
    ...
    def finalize(self, ev):
        ...
        fluid_eqs = fluid.gen_eqs(ev, port.p.s, h_outflow.s, T.s, u.s, d.s,
                                   name_prefix=f"{self.name}_{self.port_name}_")
...
class StaticPipe(FluidTwoPort):
    ...
    @staticmethod
    def get_friction_factor(Re, e, D):
        """Darcy friction factor using the Churchill (1977) unified correlation.

        Valid continuously for laminar, transitional, and turbulent flow at any
        pipe-wall roughness (e/D). Reduces to 64/Re in the laminar regime and
        matches the Moody diagram in the turbulent regime.

        References:
            Churchill, S. W. (1977). Friction-factor equation spans all fluid-flow
            regimes. *Chemical Engineering*, 84(24), 91–92.
        """
        Re_s = Re.s if hasattr(Re, "s") else Re
        e_s = e.s if hasattr(e, "s") else e
        D_s = D.s if hasattr(D, "s") else D

        # Relative roughness term (zero for smooth pipes)
        rel_roughness = sp.Piecewise((0.27 * e_s / D_s, e_s > 0), (sp.Integer(0), True))

        A = (-2.457 * sp.log((7.0 / Re_s) ** sp.Rational(9, 10) + rel_roughness)) ** 16
        B = (37530.0 / Re_s) ** 16
        return (
            8.0
            * ((8.0 / Re_s) ** 12 + (A + B) ** sp.Rational(-3, 2))
            ** sp.Rational(1, 12)
        )
...
class PTSensor(ComponentBase):
    ...
    # NOTE: The finalize() method that would add full fluid thermodynamic equations to the
    # sensor ports is intentionally omitted. When included, it adds equations like
    # p = d * Rs * T (ideal gas) which increases the DAE index and can cause solver
    # convergence issues during initialization (the Jacobian becomes ill-conditioned for
    # some circuit topologies). The sensor currently outputs pressure and enthalpy stream
    # values only; temperature output requires proper fluid property models in the
    # connected circuit's Accumulator or FluidResistor.
    #
    # To re-enable: uncomment the finalize() below, ensure all fluid networks have
    # FluidProperties components, and verify with test_fluid.py that initialization
    # converges with scale=True.
    # def finalize(self, ev):
    #     def add_fld_eqs(p):
    #         self.add_eqs(
...
class Accumulator(FluidOnePort):
    ...
    def __init__(
        ...
        if enable_enthalpy_sensor:
            ...
            self.declare_equation(
                sp.Eq(self.ports[self.port_name].h_inStream.s, sens_h_inStream.s),
                kind=EqnKind.outp,
            )
            ...
            self.declare_equation(
                sp.Eq(self.ports[self.port_name].T.s, T_fluid_out.s),
                kind=EqnKind.outp,
            )

            port_mflow = self.declare_symbol(
                ev, "port_mflow", self.name, kind=SymKind.outp
            )
            self.declare_equation(
                sp.Eq(self.ports[self.port_name].mflow.s, port_mflow.s),
                kind=EqnKind.outp,
            )

            sens_mass = self.declare_symbol(
                ev, "sens_mass", self.name, kind=SymKind.outp
            )
            self.declare_equation(
                sp.Eq(self.m.s, sens_mass.s),
                kind=EqnKind.outp,
            )

            sens_U = self.declare_symbol(
                ev, "sens_U", self.name, kind=SymKind.outp
            )
            self.declare_equation(
                sp.Eq(self.U.s, sens_U.s),
                kind=EqnKind.outp,
            )
            ...
            self.declare_equation(
                sp.Eq(self.ports[self.port_name].u.s, sens_u.s),
                kind=EqnKind.outp,
            )
...
class ThermalPipe(FluidTwoPort):
    ...
    def finalize(self, ev):
        def add_fld_eqs(p, port_name):
            self.add_eqs(
                p.fluid.gen_eqs(ev, p.p.s, p.h_outflow.s, p.T.s, p.u.s, p.d.s,
                                name_prefix=f"{self.name}_{port_name}_"),
                kind=EqnKind.fluid,
            )
            return p.h_outflow.s, p.h_inStream.s
        ...
        h_out1, h_inS1 = add_fld_eqs(p1, self.port_1_name)
        h_out2, h_inS2 = add_fld_eqs(p2, self.port_2_name)
        ...
        self.add_eqs(
            p1.fluid.gen_eqs(ev, p1.p.s, h_inS1, T_inS1.s, u_inS1.s, d_inS1.s,
                             name_prefix=f"{self.name}_{self.port_1_name}_inS_"),
            kind=EqnKind.fluid,
        )
        ...
        self.add_eqs(
            [
                ...
                # In reverse flow the fluid on port 1 exits with the enthalpy
                # it carried in from port 2 (adiabatic pass-through).
                sp.Eq(h_out1, h_inS2),
            ]
        )
