...
class Battery(ElecTwoPin):
    ...
    def __init__(
        ...
        V_ic = npa.interp(initial_soc, npa.array(OCV_soc), npa.array(OCV_volts))
        ...
        if all(params_are_scalars):
            ...
            if not len(R) == len(Rp) == len(Cp) == len(SOC_v):
                raise AcausalModelError(
...
class Diode(ElecTwoPin):
    """Shockley diode with a parallel leakage resistance.

    https://en.wikipedia.org/wiki/Shockley_diode_equation

    Characteristic (Shockley exponential with a limited exp)::

        i_d   = Ids * (limexp(V / Vt) - 1)
        i_pin = i_d + V / Rp

    Args:
        Ids (number): Saturation current in amps (> 0).
        Rp (number): Parallel leakage resistance in ohms (> 0).
        Vt (number): Thermal voltage in volts (> 0).

    Numerics (T-134):
        ``limexp`` is a SPICE-``pnjlim``-style limited exponential: it equals
        the exact ``exp(x)`` below a breakpoint ``x_max=40`` (V < 40·Vt = 1.6 V
        at the default Vt) and continues as the C¹ tangent line
        ``exp(x_max)·(1 + (x − x_max))`` above it. This keeps ``i_d`` *finite*
        in deep forward bias instead of overflowing float64 (the raw
        ``exp(V/Vt)`` hits ``inf`` at V≳28.4 V), with **no** change to behaviour
        in the normal operating range (a physical diode sits well below 1.6 V).
    ...
    def __init__(self, ev, name=None, Ids=1e-6, Rp=1e8, Vt=0.04):
        ...
        # T-134: limited (C¹) exponential to tame the Shockley stiffness. The
        # raw exp(V/Vt) is unbounded — under an AC drive the solver's Newton
        # iterates overshoot into deep forward bias where exp overflows float64
        # (V≳28V) / is catastrophically stiff, and the simulation hangs. Replace
        # it with a SPICE-`pnjlim`-style limited exponential: exact below the
        # breakpoint x_max, and a tangent-line continuation above it so the
        # current stays finite and grows only linearly in deep forward bias.
        # The continuation is C¹ (value and slope match exp(x_max) at x_max), so
        # the Newton Jacobian stays smooth. x_max=40 ⇒ exact for V<40·Vt (1.6 V
        # at the default Vt=0.04), well above any physical forward drop.
        x = self.V.s / Vt.s
        x_max = 40.0
        exp_lim = sp.Piecewise(
            (sp.exp(x_max) * (1.0 + (x - x_max)), x > x_max),
            (sp.exp(x), True),
        )
        self.add_eqs(
            [
                sp.Eq(id_.s, Ids.s * (exp_lim - 1.0)),
...
class CurrentSource(ElecTwoPin):
    ...
    Uses the Modelica passive-sign convention: ``i`` is the current flowing
    **into** the positive pin ``cs.p`` from the external circuit.  To drive
    conventional current **out** of ``cs.p`` (i.e., to inject current into a
    node connected to ``cs.p``), pass a **negative** value of ``i``.

    Example — inject 2 A into a parallel R‖C load::

        cs = CurrentSource(ev, name="cs", i=-2.0)   # drives 2 A into node
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # NOTE: CurrentSource uses the Modelica passive-sign convention.
        # Eq(self.Ip.s, i.s) sets the current flowing INTO cs.p equal to i.
        # A positive i means the source *draws* i amps from cs.p; use i < 0
        # to *supply* current to the node at cs.p.  As with any current source,
        # the voltage across it is determined by the rest of the circuit — a
        # closed loop with at least one resistive element and a Ground reference
        # is required for a structurally determined voltage.
...
class IntegratedMotor(ElecTwoPin):
    ...
    def __init__(
        ...
        # eff_spd = npa.linspace(0, npa.max(self.peaktrq_spd), 20)
        # eff_trq = npa.linspace(0, npa.max(self.peaktrq_trq), 20)
        # eff_eff = npa.ones((len(eff_spd), len(eff_trq))) * self.eff_k
        # self.eff_k = None
    ...
    def _process_peak_trq(
        ...
        if peaktrq_spd is None:
            peaktrq_spd = npa.arange(0, peak_spd, 50)
        if peaktrq_trq is None:
            if peak_pwr is None:
                peak_pwr = 100e3  # Watts
            if peak_trq is None:
                peak_trq = 200  # Nm
            peak_trq_v = npa.ones_like(peaktrq_spd) * peak_trq  # Nm
            peak_pwrTrq_v = peak_pwr / npa.maximum(peaktrq_spd, 1.0)  # Nm
            peaktrq_trq = npa.minimum(peak_trq_v, peak_pwrTrq_v)

        if peaktrq_trq[-1] != 0.0:
            # zero torque at peak speed
            peaktrq_spd = npa.append(peaktrq_spd, peaktrq_spd[-1] * 1.02)
            peaktrq_trq = npa.append(peaktrq_trq, 0.0)
...
class ACVoltageSource(ElecTwoPin):
    """
    Sinusoidal voltage source.

    V(t) = bias + amplitude * sin(2*pi*frequency*t + phase)
    """
    ...
    def __init__(
        ...
        self.name = self.__class__.__name__ if name is None else name
        super().__init__(ev, self.name)
        amplitude = self.declare_symbol(
            ev, "amplitude", self.name, kind=SymKind.param, val=amplitude
        )
        frequency = self.declare_symbol(
            ev,
            "frequency",
            self.name,
            kind=SymKind.param,
            val=frequency,
            validator=lambda f: f >= 0.0,
            invalid_msg=f"Component {self.__class__.__name__} {self.name} must have frequency>=0",
        )
        phase = self.declare_symbol(ev, "phase", self.name, kind=SymKind.param, val=phase)
        bias = self.declare_symbol(ev, "bias", self.name, kind=SymKind.param, val=bias)
        self.add_eqs(
            [
                sp.Eq(
                    self.V.s,
                    bias.s
                    + amplitude.s * sp.sin(2.0 * sp.pi * frequency.s * ev.t + phase.s),
                )
            ]
        )


class IdealSwitch(ElecTwoPin):
    """
    Controlled idealized switch with Ron/Roff resistance.
    """
    ...
    def __init__(
        ...
        control_threshold = self.declare_symbol(
            ev,
            "control_threshold",
            self.name,
            kind=SymKind.param,
        ...
        if enable_control_port:
            control = self.declare_symbol(ev, "control", self.name, kind=SymKind.inp)
        else:
            control = self.declare_symbol(
                ev, "control", self.name, kind=SymKind.param, val=control
            )

        g_cond = self.declare_conditional(
            ev,
            control.s >= control_threshold.s,
            1.0 / Ron.s,
            1.0 / Roff.s,
            cond_name="g_cond",
        )
        self.add_eqs(
            [
                sp.Eq(self.Ip.s, g_cond.s * self.V.s),
            ]
        )


class IdealTransformer(ComponentBase):
    """
    Ideal two-winding transformer.
    n is the turns ratio V2 / V1.
    """

    def __init__(self, ev, name=None, n=1.0):
        self.name = self.__class__.__name__ if name is None else name
        super().__init__()
        Vp1, Ip1 = self.declare_electrical_port(ev, "p1")
        Vn1, In1 = self.declare_electrical_port(ev, "n1")
        Vp2, Ip2 = self.declare_electrical_port(ev, "p2")
        Vn2, In2 = self.declare_electrical_port(ev, "n2")
        self.port_idx_to_name = {-1: "p1", -2: "n1", 1: "p2", 2: "n2"}
        n = self.declare_symbol(
            ev,
            "n",
            self.name,
            kind=SymKind.param,
            val=n,
            validator=lambda x: x > 0.0,
            invalid_msg=f"Component {self.__class__.__name__} {self.name} must have n>0",
        )
        v1 = self.declare_symbol(ev, "v1", self.name, kind=SymKind.var)
        v2 = self.declare_symbol(ev, "v2", self.name, kind=SymKind.var)
        self.add_eqs(
            [
                sp.Eq(v1.s, Vp1.s - Vn1.s),
                sp.Eq(v2.s, Vp2.s - Vn2.s),
                sp.Eq(v2.s, n.s * v1.s),
                sp.Eq(Ip1.s, -n.s * Ip2.s),
                sp.Eq(0, Ip1.s + In1.s),
                sp.Eq(0, Ip2.s + In2.s),
            ]
        )
...
class DCMotorSimple(ComponentBase):
    """
    Simple DC motor model (electrical + rotational) with viscous losses.
    """
    ...
    def __init__(
        ...
        self.port_idx_to_name = {-1: "pos", -2: "neg", 1: "shaft"}
        R = self.declare_symbol(ev, "R", self.name, kind=SymKind.param, val=R)
        Kt = self.declare_symbol(ev, "Kt", self.name, kind=SymKind.param, val=Kt)
        Ke = self.declare_symbol(ev, "Ke", self.name, kind=SymKind.param, val=Ke)
        J = self.declare_symbol(ev, "J", self.name, kind=SymKind.param, val=J)
        B = self.declare_symbol(ev, "B", self.name, kind=SymKind.param, val=B)
        self.add_eqs(
            [
                sp.Eq(0, Ip.s + In.s),
                sp.Eq(Vp.s - Vn.s, R.s * Ip.s + Ke.s * w.s),
                sp.Eq(-trq.s, Kt.s * Ip.s - J.s * alpha.s - B.s * w.s),
            ]
        )
