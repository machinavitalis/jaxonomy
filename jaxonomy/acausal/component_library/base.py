...
class Eqn:
    ...
    def subs(self, e1, e2):
        ...
        # After substitution, a Derivative may have a non-Function first
        # argument (e.g. Derivative(-f(t), t)) if the substitution introduced
        # a negation or linear combination.  Calling .doit() on the equation
        # normalises these so that Derivative(-f(t), t) → -Derivative(f(t), t),
        # which is required by code-generation backends.
        if hasattr(self.e, "has") and self.e.has(sp.Derivative):
            self.e = self.e.doit()
        return self
