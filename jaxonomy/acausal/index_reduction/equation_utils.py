...
def process_equations(eqs, known_vars):
    ...
    # T-002a: Sort by sympy symbol string so the returned variable lists are
    # deterministic across runs.  Earlier `list(set(...))` produced hash-order
    # results, which propagated into Pantelides graph traversal and
    # dummy-derivative selection — making the final DAE structure vary between
    # compiles of the same diagram.
    return (
        sorted(x, key=str),
        sorted(x_dot, key=str),
        sorted(y, key=str),
        sorted(X, key=str),
        vars_in_eqs,
        eqs_idx,
...
def _get_jacoboian_at_t0(t, eqs, X, ics, ics_weak, knowns, full_X=False):
    ...
    # Equivalent to knowns.keys()/knowns.values(); zip(*items()) makes the
    # key/value pairing explicit.
    ...
    if full_X:
        ...
        # When there are no strong ICs (only weak ICs), `ics` is empty and
        # zip(*ics.items()) would raise "not enough values to unpack". Fall back
        # to an empty strong-IC set so models with weak-only ICs don't crash.
        # NB: this weak-IC-only branch has no dedicated regression test yet.
        if ics:
            x, x_0 = zip(*ics.items())
        else:
            x, x_0 = (), ()
