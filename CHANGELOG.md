...
- **Acausal compile no longer hangs on large initial-condition swap searches**: when index reduction had to choose which initial conditions to weaken among many rigidly-coupled shafts / redundant fixed ICs, it enumerated every `itertools.combinations` swap, which grows combinatorially — a multi-gear drivetrain (e.g. a mid-drive e-bike with crank + motor geared to a shared wheel shaft) could take many minutes to compile, effectively hanging. When the swap search space exceeds 100 combinations, a greedy maximum-matching repair now selects a structurally- and numerically-feasible IC set directly (both the determined-IC swap and the underdetermined-set selection paths). Verified on a full multi-domain e-bike model (compile 6+ min → ~seconds) with the acausal test suite green.
- **Acausal lookup-table components work under JAX transformations**: `ComponentBase`'s lookup-table parameter validation rejected `DynamicJaxprTracer`, so components with tabulated parameters (`BatteryCellECM`, `IntegratedMotor`, tabulated resistors) raised `AcausalModelError` when constructed inside `jax.jit`/`jax.vmap`. The check is now duck-typed — sequences, or anything exposing `.shape` (numpy/JAX arrays and tracers alike) — while still rejecting scalars and strings.
- **Reverse-mode adjoint no longer skips the final checkpoint sub-interval**: the adjoint's forward pass stored the final state at `jnp.maximum(index+1, max_checkpoints-1)` — when the checkpoint array was exactly full this produced an out-of-bounds scatter that JAX silently drops, so the backward sweep never integrated the costate over the last `(ts[-1], tf]` stretch, precisely where the adjoint is largest. Whether the bug fired depended on the forward step count landing on the full-array pattern, which made the resulting gradient error look erratically tolerance- and problem-dependent (the reported symptom: `dopri5` reverse-mode ~2–3.5% wrong on a damped double pendulum at `rtol=1e-8` with `max_minor_step_size=0.01`, "converging" only near `rtol=1e-12`; BDF was ~1% wrong on other configurations for the same reason). Now `minimum` — the final state overwrites the last interior checkpoint, keeping the sweep contiguous. Also refresh the stored RHS after each checkpoint teleport (it previously carried the pre-teleport value into the first Runge–Kutta stage of every backward sub-interval). Affected all adjoint solvers (`dopri5`/`bdf`/`rk4`); gradients on the reported configuration now match finite differences to ~1e-9.: it previously crashed at construction (read `port.fluid` before the network was bound, used nonexistent media attributes, and referenced undefined `M1`/`M2` flows). It now implements the steady-state momentum balance `P_a − P_b = f(Re)·(L/D)·ṁ|ṁ|/(2ρA²) + ρ·g·h_ab` with the Churchill (1977) friction factor (continuous across laminar/turbulent, any roughness), flow reversal, optional static head, and `enable_sensors` outputs — validated against analytic Hagen–Poiseuille flow and hydrostatic equilibrium. Fluid media (`IdealGasAir`, `WaterLiquidSimple`, `WaterLiquid`) gained a documented constant `viscosity_dyn` [Pa·s]; the pipe accepts a `mu=` override and raises clearly if neither is available.
...
- **Electric-drives sub-library** (`jaxonomy.library.drives`): `PMSM` — interior permanent-magnet synchronous machine in the rotor (dq) frame with saliency (`Ld≠Lq`), torque `1.5·p·(λm·i_q + (Ld−Lq)·i_d·i_q)`, optional load-torque port and locked-rotor bench mode, state + torque outputs; `Clarke`/`InverseClarke` and `Park`/`InversePark` transform blocks (amplitude-invariant); `AveragedInverter` with the bus-voltage amplitude limit (`V_dc/√3` SVPWM or `V_dc/2` SPWM), angle-preserving clipping. Seeded from the validated inline implementations in the electric-drives tutorial series; tested against analytic RL time constants, an independent steady-state root-solve, power balance, transform round-trip identity, and inverter-limited closed-loop response.
- **Discrete-time `step_response` / `impulse_response`**: both now evaluate the exact recurrence (`lax.scan`) for a `LinearizedSystem` carrying `dt`, sampled on the integer grid `k = t/dt` — previously they raised `ValueError` on discrete input. Off-grid times raise (no silent interpolation); negative times return the causal zero; the impulse follows the `scipy.signal.dimpulse` convention (`y[0]=D`, `y[k]=C·A^{k-1}·B`). ZOH-discretized systems reproduce the continuous step response at the sample instants exactly; differentiable through `A, B, C, D`.
